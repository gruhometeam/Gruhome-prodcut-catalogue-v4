...
export default function TabBar({ cartCount = 0 }) {
    ...
    return (
      ...
      >
        <span className="relative flex">
          <Icon size={22} className={active ? 'text-[#2B2B2B]' : 'text-[#2B2B2B]/35'} />
        ...
        <span className={`text-[10.5px] ${active ? 'font-semibold text-[#2B2B2B]' : 'font-medium text-[#2B2B2B]/35'}`}>
          {label}
        </span>
        {hindiOn && (
          <span className="text-[9px] text-[#2B2B2B]/30 leading-none">{H[hindiKey]}</span>
        )}
      </button>
    );
  };
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-[#C8C2B8] flex pb-6 pt-1.5">
