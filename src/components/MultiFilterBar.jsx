...
export default function MultiFilterBar({ filterSpecs, selectedFilters, onFilterChange }) {
  ...
  return (
    <div ref={containerRef} className="w-full relative">
      <div 
        className="flex flex-row flex-nowrap overflow-x-auto gap-3 items-center py-2 -my-2"
        style={{ 
          display: 'flex',
          flexDirection: 'row',
          flexWrap: 'nowrap',
          ...
          return (
            <div key={spec.key} className="relative flex-shrink-0">
              <button
                ...
                className={`px-4 py-2 flex items-center gap-2 text-[#2B2B2B] font-medium text-[13px] bg-white border border-[#C8C2B8] rounded-lg hover:bg-[#F4ECD8] transition-all whitespace-nowrap shadow-sm ${
                  isActive ? 'border-[#8A9A5B] text-[#8A9A5B] bg-[#8A9A5B]/5' : ''
                }`}
              >
                {isActive 
